...
class TestEngine(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.graph.initialize_game(
            decklist1=[vocab.ID_CARD_FOREST, vocab.ID_CARD_GRIZZLY_BEARS],
            decklist2=[]
        )
    ...
    def test_play_land(self):
        ...
        battlefield_zone = next(self.graph.entities[r.target] for r in self.graph.get_relationships(source=self.player1, rel_type=vocab.ID_REL_CONTROLS) if self.graph.entities[r.target].type_id == vocab.ID_ZONE_BATTLEFIELD)
    ...
    def test_tap_for_mana(self):
        ...
        self.assertEqual(self.player1.properties['mana_pool'][vocab.ID_MANA_GREEN], 1, "Green mana was not added to the pool.")
    ...
    def test_cast_creature(self):
        ...
        self.graph.initialize_game(
            decklist1=[vocab.ID_CARD_FOREST, vocab.ID_CARD_FOREST, vocab.ID_CARD_GRIZZLY_BEARS],
            decklist2=[]
        )
        self.player1 = self.graph.entities[self.graph.active_player_id]
        ...
        self.assertEqual(self.player1.properties['mana_pool'][vocab.ID_MANA_GREEN], 0, "Mana was not spent correctly.")
    ...
    def test_declare_attacker(self):
        ...
        self.graph.initialize_game(decklist1=[vocab.ID_CARD_GRIZZLY_BEARS], decklist2=[])
        self.player1 = self.graph.entities[self.graph.active_player_id]
        self.engine = Engine(self.graph)
        creature_card = next(c for c in self.graph.entities.values() if c.type_id == vocab.ID_CARD_GRIZZLY_BEARS)
        
        # Manually move creature to battlefield and set its turn_entered property for the test
        battlefield_zone = next(self.graph.entities[r.target] for r in self.graph.get_relationships(source=self.player1, rel_type=vocab.ID_REL_CONTROLS) if self.graph.entities[r.target].type_id == vocab.ID_ZONE_BATTLEFIELD)
        card_zone_rel = self.graph.get_relationships(source=creature_card, rel_type=vocab.ID_REL_IS_IN_ZONE)[0]
        ...
        self.graph.step = vocab.ID_STEP_DECLARE_ATTACKERS
    ...
    def test_full_combat(self):
        ...
        self.graph.initialize_game(
            decklist1=[vocab.ID_CARD_GRIZZLY_BEARS],
            decklist2=[vocab.ID_CARD_GRIZZLY_BEARS]
        )
        self.player1 = self.graph.entities[self.graph.active_player_id]
        ...
        p1_creature = next(c for c in self.graph.entities.values() if c.type_id == vocab.ID_CARD_GRIZZLY_BEARS and self.graph.get_relationships(source=self.player1, target=c, rel_type=vocab.ID_REL_CONTROLS))
        p2_creature = next(c for c in self.graph.entities.values() if c.type_id == vocab.ID_CARD_GRIZZLY_BEARS and self.graph.get_relationships(source=self.player2, target=c, rel_type=vocab.ID_REL_CONTROLS))

        # Manually move creatures to battlefield for the test
        battlefield1 = next(self.graph.entities[r.target] for r in self.graph.get_relationships(source=self.player1, rel_type=vocab.ID_REL_CONTROLS) if self.graph.entities[r.target].type_id == vocab.ID_ZONE_BATTLEFIELD)
        battlefield2 = next(self.graph.entities[r.target] for r in self.graph.get_relationships(source=self.player2, rel_type=vocab.ID_REL_CONTROLS) if self.graph.entities[r.target].type_id == vocab.ID_ZONE_BATTLEFIELD)
        self.graph.get_relationships(source=p1_creature, rel_type=vocab.ID_REL_IS_IN_ZONE)[0].target = battlefield1.instance_id
        self.graph.get_relationships(source=p2_creature, rel_type=vocab.ID_REL_IS_IN_ZONE)[0].target = battlefield2.instance_id
        ...
        self.graph.step = vocab.ID_STEP_DECLARE_BLOCKERS
    ...
    def test_phase_and_step_progression(self):
        ...
        # Initial state
        self.assertEqual(self.graph.phase, vocab.ID_PHASE_BEGINNING)
        self.assertEqual(self.graph.step, vocab.ID_STEP_UNTAP)
        ...
        for _ in range(len(vocab.PHASE_ORDER) * 3): # Iterate enough times to cover multiple turns
            current_phase = self.graph.phase
            current_step = self.graph.step
            current_phase_steps = vocab.PHASE_STEPS.get(current_phase, [])
            ...
            if current_step_index < len(current_phase_steps) - 1:
                ...
                expected_next_phase_index = (vocab.PHASE_ORDER.index(current_phase) + 1) % len(vocab.PHASE_ORDER)
                expected_next_phase = vocab.PHASE_ORDER[expected_next_phase_index]
                self.assertEqual(self.graph.phase, expected_next_phase)
                
                expected_next_step = vocab.PHASE_STEPS.get(expected_next_phase, [])[0]
                self.assertEqual(self.graph.step, expected_next_step)

            # Check turn number and active player change after a full cycle
            if self.graph.phase == vocab.ID_PHASE_BEGINNING and self.graph.step == vocab.ID_STEP_UNTAP and self.graph.turn_number > initial_turn_number:
